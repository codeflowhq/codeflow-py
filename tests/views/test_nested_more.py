...
def test_render_inline_child_view_returns_html_label_for_single_html_node(monkeypatch: pytest.MonkeyPatch) -> None:
    from code_visualizer import graph_view_builder

    def _fake_build(*args: object, **kwargs: object) -> tuple[str, VisualGraph]:
        graph = VisualGraph()
        graph.add_node(VisualNode("root", NodeKind.OBJECT, "<b>x</b>", {"html_label": True}))
        return "root", graph

    monkeypatch.setattr(graph_view_builder, "build_graph_view", _fake_build)
    assert render_inline_child_view(_runtime(), {"a": 1}, "slot", ViewKind.TABLE, 2) == "<b>x</b>"


def test_render_inline_child_view_returns_none_on_build_or_render_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    from code_visualizer import graph_view_builder
    from code_visualizer.views import nested

    monkeypatch.setattr(graph_view_builder, "build_graph_view", lambda *args, **kwargs: (_ for _ in ()).throw(ValueError("boom")))
    assert render_inline_child_view(_runtime(), [1, 2], "slot", ViewKind.ARRAY_CELLS, 2) is None
    ...
    monkeypatch.setattr(nested, "_render_dot_to_image", lambda dot_source, fmt="png": None)
    assert render_inline_child_view(_runtime(), [1, 2], "slot", ViewKind.ARRAY_CELLS, 2) is None


def test_make_nested_renderer_returns_none_or_adds_edge(monkeypatch: pytest.MonkeyPatch) -> None:
    runtime = _runtime()
    parent_id = "parent"
    ...
    monkeypatch.setattr(nested, "render_inline_child_view", lambda *args, **kwargs: None)
    monkeypatch.setattr(nested, "select_nested_view", lambda *args, **kwargs: ViewKind.TABLE)
    monkeypatch.setattr(graph_view_builder, "_build_view", lambda runtime, value, name, view, depth: "child")
